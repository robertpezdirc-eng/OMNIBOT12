...
class ModuleManager {
    ...
    updateModuleStatus() {
        ...
        // Preveri veljavnost licence - samodejno blokiranje po expires_at
        if (this.currentLicense.expiresAt && Date.now() > this.currentLicense.expiresAt) {
            console.log(`⏰ Licenca ${this.currentLicense.type} je potekla (${new Date(this.currentLicense.expiresAt).toLocaleString()}), zaklepanje vseh modulov...`);
            
            // Zakleni vse module zaradi poteka licence
            Object.keys(this.modules).forEach(moduleId => {
                this.modules[moduleId].status = 'expired';
            });

            // Nastavi licenco kot poteklo
            this.currentLicense.status = 'expired';
            
            // Pošlji dogodek o poteku licence
            if (typeof window !== 'undefined' && window.electronAPI) {
                window.electronAPI.send('license-expired', {
        ...
        console.log('Status modulov posodobljen za licenco:', this.currentLicense.type, 
                   this.currentLicense.status === 'expired' ? '(POTEKLA)' : '');
    }
